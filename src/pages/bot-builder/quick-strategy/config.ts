...
export const STRATEGIES = (): TStrategies => ({
    ...
    DIGIT_PREDICTION_RECOVERY: {
        name: localize('Digit Prediction Recovery'),
        description: DIGIT_PREDICTION_RECOVERY,
        fields: [
            ['INITIAL_STAKE', STAKE()],
            ['MARTINGALE_SIZE', SIZE()],
            ['MAX_RECOVERY_STEPS', { min: 1, max: 10, increment: 1 }],
            ['LOSS', LOSS()],
        ],
        should_show: true,
    },
