...
export class DerivTickTrendAnalyzer {
    ...
    generateTradingDecision(symbol: string): {
        ...
        return {
            ...
            confidence: signal.confidence,
            reasoning: [...signal.reasons, `Market: ${marketCondition.reason}`],
            riskLevel,
    ...
    getDetailedAnalytics(symbol: string): {
        ...
        return {
            currentAnalysis: this.analysisCache.get(symbol) || null,
            marketCondition: this.getMarketCondition(symbol),
            recentSignals: this.signalHistory.get(symbol)?.slice(-5) || [],
            ticksAnalyzed: this.tickHistory.get(symbol)?.length || 0,
            signalAccuracy: this.calculateSignalAccuracy(symbol)
        };
    }

    private calculateSignalAccuracy(symbol: string): number {
        const signals = this.signalHistory.get(symbol);
        if (!signals || signals.length < 5) return 0;

        // Simple accuracy calculation based on signal strength vs confidence
        const validSignals = signals.filter(s => s.confidence > 60);
        if (validSignals.length === 0) return 0;

        const strongSignals = validSignals.filter(s => s.strength === 'STRONG').length;
        return (strongSignals / validSignals.length) * 100;
    }

    /**
     * Reset analyzer for a symbol
     */
    ...
    getSystemStatus(): {
        ...
        return {
            activeSymbols: this.tickHistory.size,
            totalTicks,
            totalSignals,
...
export const tickTrendAnalyzer = new DerivTickTrendAnalyzer();
